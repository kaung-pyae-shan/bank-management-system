...
public class AccountNumberGenerator {
	...
	public static String generateNewAccountNumber(Type accoutType, OwnershipType ownershipType) {
		...
		if (lastAccountNumber != null) {
			int newNumberInt = Integer.parseInt(lastAccountNumber) + 1; // Increment the last number
			newNumber = Integer.toString(newNumberInt);
		} else {
//			11 for Saving
//			22 for Fixed
//			01 for Single
//			02 for Joint
			...
			} else if(accoutType == Type.FIXED && ownershipType == OwnershipType.SINGLE) {
				newNumber = "220100000001";
			} else if(accoutType == Type.FIXED && ownershipType == OwnershipType.JOINT) {
